...
const intToFloat = (nums: number, decimalPlaces: number) => {
  ...
  return hasMinus?-Number([...s2, '.', ...s3, s1[1] ?? ''].join('')):Number([...s2, '.', ...s3, s1[1] ?? ''].join(''));
};

// 计算
export const compute = (num1: number, num2: number, symbol: Symbol) => {
...
// 小数计算方法 注：不支持()
export const calculate = (arg0: any, ...args: any) => {
  try {
    // 过滤不符合规则的参数
    ...
    const diGuiCompute = (arg0: any, args: number[]): number => {
      if (args.length === 1) {
        ...
        args.splice(
          index,
          2,
          compute(args[index], args[index + 1], arg0[index]),
        );
        arg0.splice(index, 1);
